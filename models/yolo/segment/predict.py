...
def predict(cfg=DEFAULT_CFG, use_python=True):
    ...
    model = cfg.model or 'seg.pt'
    # source = cfg.source if cfg.source is not None else ROOT / 'assets' if (ROOT / 'assets').exists() \
    #     else 'https://ultralytics.com/images/bus.jpg'
    source = r'SegmentBorder3/test/images'

    args = dict(model=model, device='0', epochs=300, batch=8, workers=4, cos_lr=True,
                amp=True, boxes=False, retina_masks=True, mode='val', conf=0.5, imgsz=[640, 640])
    if use_python:
        from ultralytics import YOLO
        seg_model = YOLO(model)
        seg_model.export(format="onnx", device='cpu')
        # seg_model(**args)
    else:
        predictor = SegmentationPredictor(overrides=args)
        predictor.predict_cli(source=source, model=model)
