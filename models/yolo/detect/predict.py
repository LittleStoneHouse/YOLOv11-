...
def predict(use_python=False):
    """Runs YOLO model inference on input image(s)."""
    model = 'F:/PythonProjects/YOLOv11/ultralytics/models/yolo/detect/y16n.pt'
    source = 'F:/PythonProjects/YOLOv11/ultralytics/models/yolo/detect/ABS/外部验证集汇总'

    args = dict(model=model, device='0', source=source, task='detect', mode='predict', boxes=True)
    if use_python:
        from ultralytics import YOLO
        pre_model = YOLO(model)
        # pre_model(**args)
        pre_model.predict(source)
    else:
        predictor = DetectionPredictor(overrides=args)
        predictor.predict_cli()
